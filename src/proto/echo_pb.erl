...
-export_type(['HeartBeatReq'/0, 'HeartBeatResp'/0]).

-spec encode_msg(#'HeartBeatReq'{} | #'HeartBeatResp'{}) -> binary().
encode_msg(Msg) when tuple_size(Msg) >= 1 ->
    encode_msg(Msg, element(1, Msg), []).

-spec encode_msg(#'HeartBeatReq'{} | #'HeartBeatResp'{}, atom() | list()) -> binary().
...
encode_msg_HeartBeatResp(#'HeartBeatResp'{now_ms = F1},
			 Bin, TrUserData) ->
    begin
...
dfp_read_field_def_HeartBeatResp(<<8, Rest/binary>>, Z1,
				 Z2, F@_1, TrUserData) ->
    d_field_HeartBeatResp_now_ms(Rest, Z1, Z2, F@_1,
				 TrUserData);
dfp_read_field_def_HeartBeatResp(<<>>, 0, 0, F@_1, _) ->
    #'HeartBeatResp'{now_ms = F@_1};
...
dg_read_field_def_HeartBeatResp(<<0:1, X:7,
    ...
    case Key of
      8 ->
	  d_field_HeartBeatResp_now_ms(Rest, 0, 0, F@_1,
				       TrUserData);
      _ ->
	  case Key band 7 of
	    0 ->
...
d_field_HeartBeatResp_now_ms(<<1:1, X:7, Rest/binary>>,
			     N, Acc, F@_1, TrUserData)
    when N < 57 ->
    d_field_HeartBeatResp_now_ms(Rest, N + 7, X bsl N + Acc,
				 F@_1, TrUserData);
d_field_HeartBeatResp_now_ms(<<0:1, X:7, Rest/binary>>,
			     N, Acc, _, TrUserData) ->
...
merge_msg_HeartBeatResp(#'HeartBeatResp'{},
			#'HeartBeatResp'{now_ms = NFnow_ms}, _) ->
    #'HeartBeatResp'{now_ms = NFnow_ms}.
...
v_msg_HeartBeatResp(#'HeartBeatResp'{now_ms = F1}, Path,
		    TrUserData) ->
    v_type_uint64(F1, [now_ms | Path], TrUserData), ok;
...
get_msg_names() -> ['HeartBeatReq', 'HeartBeatResp'].
...
fqbin_to_msg_name(<<"HeartBeatReq">>) -> 'HeartBeatReq';
fqbin_to_msg_name(<<"HeartBeatResp">>) -> 'HeartBeatResp';
fqbin_to_msg_name(E) -> error({gpb_error, {badmsg, E}}).


msg_name_to_fqbin('HeartBeatReq') -> <<"HeartBeatReq">>;
msg_name_to_fqbin('HeartBeatResp') -> <<"HeartBeatResp">>;
...
get_all_source_basenames() -> ["echo.proto"].
...
%% functions.
get_all_proto_names() -> ["echo"].


get_msg_containment("echo") ->
    ['HeartBeatReq', 'HeartBeatResp'];
get_msg_containment(P) ->
    error({gpb_error, {badproto, P}}).


get_pkg_containment("echo") -> undefined;
get_pkg_containment(P) ->
    error({gpb_error, {badproto, P}}).


get_service_containment("echo") -> [];
get_service_containment(P) ->
    error({gpb_error, {badproto, P}}).


get_rpc_containment("echo") -> [];
get_rpc_containment(P) ->
    error({gpb_error, {badproto, P}}).


get_enum_containment("echo") -> [];
get_enum_containment(P) ->
    error({gpb_error, {badproto, P}}).


get_proto_by_msg_name_as_fqbin(<<"HeartBeatResp">>) -> "echo";
get_proto_by_msg_name_as_fqbin(<<"HeartBeatReq">>) -> "echo";
